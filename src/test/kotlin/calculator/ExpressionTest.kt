...
class ExpressionTest {
    ...
    fun `올바르지 않은 계산식 입력하면 예외가 발생한다`(wrongException: Input) {
        assertThat(
            assertThrows<IllegalArgumentException> {
                Expression(InputView(wrongException))
            }.message,
    ...
    fun `올바른 계산식을 입력하면 피연산자를 분리 할 수 있다`() {
        val expression = Expression(InputView(Input("2 * 9 / 3")))
    ...
    fun `올바른 계산식을 입력하면 연산자를 분리 할 수 있다`() {
        val expression = Expression(InputView(Input("18 / 3 * 8")))
    ...
    companion object {
        ...
        fun providedWrongExpression() =
            listOf(
                Arguments.arguments(Input("1 3 7 50 + * -")),
                Arguments.arguments(Input("x 1 5 2xxx")),
                Arguments.arguments(Input("101 99 +")),
            )
    }
}
